...
int main(int argc, char* argv[]) {
    ...
    const int net_layers = 4;
    const int elements_foreachbeam = 56;
    const double grid_spacing = 0.05;
    const double layer_spacing_start = wire_diameter;
    ...
    {    
        ...
        {    
            ...
            ChVector<> beam_pos_start((beam_sel+1)*grid_spacing-0.5*(beam_num+1)*grid_spacing, layer_sel*layer_spacing_start, -0.5*(beam_num+1)*grid_spacing);
            ChVector<> beam_pos_end((beam_sel+1)*grid_spacing-0.5*(beam_num+1)*grid_spacing, layer_sel*layer_spacing_start, +0.5*(beam_num+1)*grid_spacing);
            ...
            ////test_force_constraint[beam_sel_x] = startConstr;

            // fix beam to end flange
            auto endConstr = std::make_shared<ChLinkMateGeneric>();
            endConstr->Initialize(endNode, fixedBody, false, endNode->Frame(), endNode->Frame());
