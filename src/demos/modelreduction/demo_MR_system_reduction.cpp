...
int main(int argc, char* argv[]) {
    ...
    //    abearing->Initialize(builder.GetLastBeamNodes()[i], my_deadbody,
    //    ChFrame<>(builder.GetLastBeamNodes()[i]->GetPos())); my_system.Add(abearing);
    //}

    // falling object
    auto floorBody = chrono_types::make_shared<ChBodyEasyBox>(0.1, 0.2, 0.3, 3000, true, false);
    ...
    */
    ...
    if (enable_eigenanalysis) {
        ...
        std::function<void()> pre_sim_fun =
            std::bind(&ChIrrApp::BeginScene, &application, true, true, irr::video::SColor(255, 0, 0, 0));
